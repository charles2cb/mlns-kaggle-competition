...
def build_final_predictions(base_pred: Dict[str, np.ndarray]) -> Dict[str, np.ndarray]:
    # No-catboost meta branch (text + graph + embedding predictors).
    no_catboost_meta = rank_average(
        [
    ...
    # CatBoost-assisted blend used by the historical I-variant.
    catboost_assisted_blend = rank_average(
        [base_pred["catboost"], base_pred["node2vec_linear"], base_pred["text_mlp"]],
        [0.55, 0.30, 0.15],
    )

    # Final kept leaderboard file: cat + graph + n2v rank average.
    rankavg_g2_catgraphn2v = rank_average(
        [base_pred["catboost"], base_pred["node2vec_linear"], base_pred["graph_linear"]],
        [0.70, 0.20, 0.10],
    )

    crossrun_rank_meta = rank_average(
        [no_catboost_meta, catboost_assisted_blend, rankavg_g2_catgraphn2v],
        [0.50, 0.30, 0.20],
    )

    crossrun_rank_meta_aggressive = rank_average(
        [crossrun_rank_meta, rankavg_g2_catgraphn2v],
        [0.40, 0.60],
    )

    return {
        "submission_crossrun_rank_meta_r2_w50_30_20_proba.csv": crossrun_rank_meta,
        "submission_crossrun_rank_meta_r2_aggr_w40_cat60_proba.csv": crossrun_rank_meta_aggressive,
    }
